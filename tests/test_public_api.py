...
from tse.public_api import MAX_BYTES, create_public_app
...
class FakeExtractor:
    ...
    def info(self):
        return {"ready": True, "training_updates": 135525}

    def extract_array(self, mixture, reference):
        return mixture * 0.5


def recordings():
    wave = wav_bytes((0.1 * np.sin(np.arange(64000) / 11)).astype(np.float32))
...
def test_service_requires_configured_token(monkeypatch):
    ...
    with TestClient(create_public_app("unused", FakeExtractor)) as client:
        ...
        assert (
            client.get("/model", headers={"Authorization": "Bearer private-test-key"}).status_code
            == 200
        )
