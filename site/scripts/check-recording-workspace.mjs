...
const {projects, projectWriter} = await import('../src/recordingProjects.js');
...
const saving = writer.save(project);
const deleting = writer.delete(project.id);
const lateAutosave = writer.save({...project, updated: 2});
await Promise.all([saving, deleting, lateAutosave]);
assert.equal(await projects('get', project.id), undefined, 'A late autosave cannot recreate a deleted recording');
assert.deepEqual(await projects('list'), []);
await writer.save({...project, id: 'another'});
assert.equal((await projects('get', 'another')).id, 'another', 'Other projects still save after deleting one');
await writer.delete('another');
console.log('Deletion wins over pending and late autosaves without affecting other projects.');
