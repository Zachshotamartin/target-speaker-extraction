        ...
        if request.method == "POST":
            ...
            async for chunk in request.stream():
                ...
                if len(body) > MAX_BYTES:
                    return Response(
                        json.dumps({"detail": "Combined upload must be under 4 MiB."}),
                        status_code=413,
    ...
    async def extract(request: Request):
        ...
        try:
            ...
            result = await run_in_threadpool(
                request.app.state.extractor.extract_array, mixture, reference
            )
            gain = min(1.0, 0.98 / max(float(np.abs(result).max()), 1e-8))
            return Response(
                wav_bytes(result * gain),
                media_type="audio/wav",
                headers={
                    "X-Checkpoint-SHA256": request.app.state.extractor.checkpoint_hash,
                    "Content-Disposition": 'attachment; filename="extracted-voice.wav"',
                },
            )
