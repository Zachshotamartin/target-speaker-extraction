...
The active direction is a [small concept demo](docs/CONCEPT_DEMO.md), with **600 updates and a 30-minute default session limit**. It adapts our own weights to eight familiar voices, using changing clean mixtures and reserving separate recordings for evaluation. Three separator blocks, three-second references and fixed-shape execution measured about 2.57 seconds per update in a short speed probe; quality must be checked separately. [Live concept progress](http://127.0.0.1:8000/experiments/concept/) shows actual completed work and development scores.

The [full-size independent reference baseline](docs/REFERENCE_BASELINE.md) remains on hold: its preliminary 100-epoch estimate was roughly 800 Mac hours. Its completed learning check reached 15.42 dB improvement on sixteen repeated training examples, which is memorization evidence, not a held-out score. The concept experiment reuses that project's checkpoint, excludes its training recordings and makes a narrower known-voice claim. The [compact v3 experiments](docs/V3_WORKLOG.md) remain preserved and paused. The default app still serves v0.2.0.
