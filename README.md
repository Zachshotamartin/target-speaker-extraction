...
The active research direction is a [full-size independent reference baseline](docs/REFERENCE_BASELINE.md): a 28.1M-parameter BSRNN/ResNet34 model, official Libri2Mix mixtures, a simpler objective and explicit epoch-based training. Its 256-update fixed-set learning check passed, reaching 15.42 dB improvement at the best checkpoint with all 16 correct source preferences. This is memorization evidence, not a held-out score. Full training is **on hold for a practical compute choice**: the preliminary 100-epoch Mac estimate is roughly 800 hours, which exceeds the user's acceptable local workload. [Live reference progress](http://127.0.0.1:8000/experiments/reference/) distinguishes the tiny check from full-data epochs. The [compact v3 experiments](docs/V3_WORKLOG.md) remain preserved and paused. The app still serves v0.2.0; no new model is deployed.
